...
const API_URL = 'http://192.168.1.14:5141';
...
export const postUsuario = async (cliente: Cliente,confirmPassword : string,celulares : celular[]) => {
  try {
    ...
    await axios.post('http://192.168.1.14:5141/api/Values/CreateUser', clientUser);

    // if(result == null)
    //   return;
    await axios.post('http://192.168.1.14:5141/api/Cliente', cliente);
    // const clienteID : any =
    // if(clienteID != 0){
...
export const cadastrarFazenda = async (fazenda : fazendaCadastro) => {
  TokenAutorizado = await AsyncStorage.getItem('authToken');
    try {
    const response = await axios.post('http://192.168.1.14:5141/api/Fazenda', fazenda, {
      headers: {
        Authorization: `Bearer ${TokenAutorizado}`,
        Accept: 'application/json'
      },
...
export const loginUser = async (cliente: LoginUser) => {
  try {
    const response = await axios.post('http://192.168.1.14:5141/api/Values/LoginUser', cliente);
